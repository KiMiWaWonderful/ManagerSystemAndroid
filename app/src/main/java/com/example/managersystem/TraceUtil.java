...
public class TraceUtil {
    ...
    public void drawHistoryTrack(BaiduMap baiduMap, List<LatLng> points, SortType sortType) {
        ...
        if (points.size() == 1) {
            OverlayOptions startOptions = new MarkerOptions().position(points.get(0)).icon(BitmapDescriptorFactory.fromResource(R.drawable.icon_start))
        ...
        OverlayOptions endOptions = new MarkerOptions().position(endPoint)
                .icon(BitmapDescriptorFactory.fromResource(R.drawable.icon_end)).zIndex(9).draggable(true);
